...
import { Heart, ShieldCheck, Copy, Check, Users, KeyRound, LogOut, Download } from "lucide-react";
    ...
    <>
      <header className="sticky top-0 z-40 bg-[#0A0518]/80 backdrop-blur-2xl border-b border-white/10 px-4 py-3 text-white">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          {/* Active Partner Identity Badge & Install App */}
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap justify-center">
            <button
              onClick={handleInstallClick}
              className="px-3 py-1.5 rounded-xl bg-pink-500/20 hover:bg-pink-500/30 text-pink-200 border border-pink-500/40 text-xs font-semibold flex items-center gap-1.5 transition-all shadow-sm active:scale-95 cursor-pointer"
              title="Install Orbit as App"
            >
              <Download className="w-3.5 h-3.5 text-pink-300 animate-bounce" />
      ...
      {showInstallModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
          <div className="bg-[#120a2a] border border-white/15 rounded-3xl max-w-md w-full p-6 text-white shadow-2xl relative">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-bold">Install Orbit on your device</h3>
                <p className="text-xs text-slate-300">Enjoy full-screen companion experience</p>
              </div>
            </div>

            <div className="space-y-3 text-sm text-slate-300 bg-white/5 p-4 rounded-2xl border border-white/10">
              <p className="font-medium text-pink-300">How to install:</p>
              <ul className="list-disc list-inside space-y-1.5 text-xs">
                <li><strong>Chrome / Edge (Android & Desktop):</strong> Click the browser menu (three dots) and select <strong>"Install Orbit"</strong> or <strong>"Add to Home screen"</strong>.</li>
                <li><strong>Safari (iOS / iPhone):</strong> Tap the <strong>Share</strong> button at the bottom of Safari, then scroll down and tap <strong>"Add to Home Screen"</strong>.</li>
              </ul>
            </div>
            ...
            <div className="mt-6 flex justify-end">
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-pink-500 to-purple-600 hover:opacity-90 text-white font-semibold text-sm rounded-xl shadow transition-all"
              >
                Got it
              </button>
            </div>
