...
import { Utensils, Smile, Zap, Heart, Sparkles, Gamepad2, Moon, Edit3, Check, HeartHandshake, UserCheck } from "lucide-react";
    ...
    <div id="tamagotchi-sprout-card" className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[32px] p-6 sm:p-8 text-white shadow-2xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
        ...
        <button
          ...
          className="px-4 py-2 rounded-2xl bg-white/10 hover:bg-white/15 text-white border border-white/15 text-xs font-semibold flex items-center justify-center gap-2 transition-all shadow-md active:scale-95"
        >
          <Edit3 className="w-4 h-4 text-emerald-400" />
          <span>Set My ({myFirstName}) Mood & Status</span>
        </button>
      </div>

      {/* Mood Setting Form Collapsible Drawer */}
      {isEditingMood && (
